...
async function refreshPage() {
  ...
  try {
    await refreshUsers()
    setNotice('ユーザー一覧を更新しました')
  }
  catch (error: any) {
    setError(error?.data?.message || error?.data?.statusMessage || 'ユーザー一覧の更新に失敗しました')
  }
  finally {
...
<template>
  <AppAuthenticatedShell
    title="ユーザーマスター"
    description="管理者ユーザーのみが利用できる画面です。ログインユーザーの追加、権限変更、削除をここで管理します。"
  >
    <template #badges>
